...
contract loteria is ERC20, Ownable {
    ...
    function createUser (
        ...
        string memory _aliasx
    ) public returns (address) {
        require(usersInit[_add] == false, "El usuario ya existe");
    ...
    function getUser(address _add) public view returns (
        string memory name,
        string memory document,
        string memory aliasx)
    {
        require(usersInit[_add] == true, "La cuenta no existe");
        User memory u = users[_add];
        return (
            u.name,
            u.document,
            u.aliasx
    ...
    function transferenciaEntreCuentas(address _to, uint256 _numTokens) public payable{
